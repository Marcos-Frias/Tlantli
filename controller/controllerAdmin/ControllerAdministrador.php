...
class ControllerAdministrador {
	...
	public function showGrado(){
		$grado = "";
		$grados = $this-> modelAdmin->showGradoDB();
	...
	public function showGrupo(){
		$grupo = "";
		$grupos = $this-> modelAdmin->showGrupoDB();
	...
	public function showCargo(){
		$cargo = "";
		$cargos = $this-> modelAdmin->showCargoDB();
	...
	public function showMaterias(){
		$materia = "";
		$materias = $this-> modelAdmin->showMateriasDB();
	...
	public function showDocentes(){
		$docente = "";
		$docentes = $this-> modelAdmin->showDocentesDB();
	...
	public function showPeriodo(){
		$periodo = "";
		$year = "";
		$periodos = $this-> modelAdmin->showPeriodoDB();
	...
	public function insertUser(){
		...
		if (isset($_POST["nickName"]) && isset($_POST["pass"]) ) {
			...
			$query = $this->modelAdmin->insertUserDB($datosUser);
 			if ($query == TRUE) {
	...
	public function insertDocente(){
		if( isset($_POST["nameDoc"]) && isset($_POST["matriculaDoc"]) ){
			...
			$query = $this->modelAdmin->insertDocenteDB($datosDoctor);
			if($query == TRUE){
	...
	public function insertMateria(){
		if( isset($_POST["nameMateria"]) && isset($_POST["grado"]) ){
			...
			$query = $this->modelAdmin->insertMateriaDB($datosMateria);
			if($query == TRUE){
	...
	public function asignarMateria(){
		...
		if( isset($_POST["docente"]) && isset($_POST["materia"]) && isset($_POST["periodoAcad"])  ){
			$docente = $this->modelAdmin->idDocenteDB($_POST["docente"]);
			$materia = $this->modelAdmin->idMateriaDB($_POST["materia"]);
			$periodo = $this->modelAdmin->idPeriodoDB($_POST["periodoAcad"]);
			if($docente != ""  and $materia != "" and $periodo != ""){
				...
				$query = $this->modelAdmin->insertAsignarMateriaDB($datos);
				if($query == TRUE){
					//header("location: ViewRegMat.php");
	...
	public function insertAlumno(){
		...
		if (isset($_POST["nameAlumno"]) && isset($_POST["matriculaAlumno"]) ) {
			...
			$query = $this->modelAdmin->insertAlumnoDB($datosAlumno);
 			if ($query == TRUE) {
